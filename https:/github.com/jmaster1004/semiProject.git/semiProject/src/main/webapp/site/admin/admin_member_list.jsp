...
<div id="wrap">
	<div class="site-wrap">
					...
					<% if(memberList.size()==0) { %>
						...
						<tr>
							<%if(member.getMemberStatus()==0){ %>
								<td id="email"><%= member.getMemberEmail() %></td>
								<td id="name"><strong><%= member.getMemberName() %></strong></td>
								<td class="subject" style="text-align:left" id="phone"><%= member.getMemberPhone() %></td>
								<td id="totalPrice">미구현</td>
								<td>
							...
							<%} else if(member.getMemberStatus()==11) {%>
								<td id="email" style="text-decoration: line-through;"><%= member.getMemberEmail() %></td>
								<td id="name" style="text-decoration: line-through;"><%= member.getMemberName() %></td>
								<td class="subject" style="text-align:left; text-decoration: line-through;" id="phone"><%= member.getMemberPhone() %></td>
								<td id="totalPrice" style="text-decoration: line-through;">미구현</td>
								<td>
							 		<button type="button" id="update" onclick= memberUpdate(this)>수정</button><br>
							 		<button type="button" id="delete"  onclick=prompttest(this) disabled style="text-decoration: line-through;">삭제</button>
								</td>
							<%} %>
						</tr>
						<% } %>
		...
		<div
			class="xans-element- xans-board xans-board-paging-4 xans-board-paging xans-board-4 ec-base-paginate">
				<% if(startPage>blockSize) { %>
					<a href="<%=request.getContextPath()%>/site/index.jsp?workgroup=admin&work=admin_member_list&pageNum=<%=startPage-blockSize%>&search=<%=search%>&keyword=<%=keyword%>"><img
				...
				<% for(int i=startPage;i<=endPage;i++) { %>
					<% if(pageNum!=i) {//요청 페이지 번호와 출력 페이지 번호가 같지 않은 경우 링크 설정 %>
						<ol>
							...
							<a href="<%=request.getContextPath()%>/site/index.jsp?workgroup=admin&work=admin_member_list&pageNum=<%=i%>&search=<%=search%>&keyword=<%=keyword%>"
								class="other"><%=i %></a></li>
						</ol>
				...
				<% if(endPage!=totalPage) {//종료 페이지 번호와 전체 페이지 갯수가 같지 않은 경우 링크 설정 %>
					<a href="<%=request.getContextPath()%>/site/index.jsp?workgroup=admin&work=admin_member_list&pageNum=<%=startPage+blockSize%>&search=<%=search%>&keyword=<%=keyword%>">
		...
		<form id="boardSearchForm" action="<%=request.getContextPath()%>/site/index.jsp?workgroup=admin&work=admin_member_list" method="post" target="_top">
